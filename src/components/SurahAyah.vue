    ...
    <b-container class="col-lg-6 col-md-12 col-sm-12">
      <b-tabs content-class="mt-3" fill>
        ...
        <b-tab title="القارئ">
          ...
          >
            <b-button
              v-for="(item, index) in arrayAudio"
              :key="index"
              :variant="
                item.identifier == $parent.settings.reader
                  ? 'primary'
                  : 'outline-primary'
              "
              @click="reReader(item.identifier)"
              :v-if="item.identifier != null"
            >
              {{ item.name }}
            </b-button>
          </carousel>
      ...
      <b-overlay
        :show="show"
        rounded="sm"
        style="line-height: initial; padding: 0 8px 15px 8px;"
      >
        <span
          v-for="(value, index) in surah"
          :key="index"
          class="ayahk"
          :data-id="value.number"
          :id="'button' + value.number"
          :aria-hidden="show ? 'true' : null"
        >
          <div
            v-if="index == 0 || value.numberInSurah == 1"
            :class="value.numberInSurah == 1 ? 'in-surah' : 'of-surah'"
          >
            <b>{{ value.surah.number }}.</b> {{ value.surah.name }}
          </div>

          <strong
            v-if="par == 1 && value.numberInSurah == 1"
            v-html="value.text"
            @click="xplay(index)"
            :class="index == tRead ? 'x-start' : ''"
          ></strong>
          <strong
            v-else
            :inner-html.prop="value.text | basmala"
            @click="xplay(index)"
            :class="index == tRead ? 'x-start' : ''"
          ></strong>
...
export default {
  ...
  data() {
    return {
      ...
      options: [
        ...
        {
          text: "سيتم الانتقال الى الصفحة التالية بعد الأنتهى من قراءة الصفحة",
          value: "next",
        },
      ],
    };
  ...
  methods: {
    async run() {
      ...
      await axios
        .get(`http://api.alquran.cloud/v1/page/${Nxpage}/${Qar}`)
        .then((response) => {
          this.surah = [];
